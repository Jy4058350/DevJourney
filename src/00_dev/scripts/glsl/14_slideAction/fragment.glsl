...
float calculateAngle(vec2 uv, vec2 center,float uProgress ) {
...
//     angle = mod(angle, 360.0);
    ...
    float angle = calculateAngle(uv, center, uProgress);
 ...
 // Blend between the current and next textures
    ...
    if(distance < radius) {
        ...
        if(distance < radius1) {
            if(angle >= 0.0 && angle < calculateAngle(uv, center, uProgress)) {
